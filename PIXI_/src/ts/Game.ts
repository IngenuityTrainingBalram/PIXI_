...
export class Game {
    ...
    // private createSnakeBode(): any {
    //     const img = new Sprite(getTexture('body') as Texture<Resource>);
    //     img.anchor.set(0.5);
    //     img.scale.set(0.15);
    //     let x = this.snake.RenderTexture(this.snake, img)
    //     return this.background.addChild(x)
    // }
    public update(): void {
        if (this.snake) {
            ...
            if (this.collision(this.snake, this.food)) {
                ...
                this.food = this.createImageFood();
                // this.Sbody = this.createSnakeBode()
                // this.snake = this.snake.RenderTexture(this.Sbody)
